<?php if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly    
	...
	$_total_posts = $this->getTotalProducts( $category_id, 1, 0 );
	if( $_total_posts <= 0 ) {
	...
	if( ( $all_pg ) >= $cur_all_pg + 2 ) {
		?>
			<div class="clr"></div>
			<div class='ik-post-load-more' align="center" onclick='WCPT_loadMoreProducts("<?php echo esc_js( $category_id ); ?>", "<?php echo esc_js( $_limit_start+$_limit_end ); ?>","<?php echo esc_js( $params["vcode"]."-".$category_id ); ?>","<?php echo esc_js( $_total ); ?>",request_obj_<?php echo esc_js( $params["vcode"] ); ?>)'>
